...
> **歴史的スナップショット:** §4–§5 は、依存パッケージの公開 API と runtime dependency を
> 整備する前に行った実測記録である。現在は `mc-sim` / `mc-kernel` / `mc-worldgen` などを
> `package.json` に明示し、公開サービスをホストから直接配線している。現行の未完了項目は
> [docs/README.md](./README.md) と [testing.md](./testing.md) の browser gate を参照する。
> 以下の「未公開」「import できない」という記述は、当時の状態を保存したものとして読む。
